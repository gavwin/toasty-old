...
module.exports = class ShardInfoCommand extends Command {
  ...
  async run(msg) {
    let uptimeRes = await this.client.shard.fetchClientValues('uptime');
    let uptime = uptimeRes.reduce((prev, val) => prev + val, 0);
    let averageUptime = uptime / this.client.shard.count;
