...
module.exports = class SkipSongCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (!queue.songs[0].dispatcher) return msg.reply('the song hasn\'t begun yet!'); // eslint-disable-line max-len
    ...
    const force = threshold <= 1
      ...
      || ((this.client.isOwner(msg.author)
        || msg.guild.roles.has(msg.guild.settings.get('dj'))
        ? this.hasDJRole(msg.author, msg)
        : msg.member.hasPermission('MANAGE_MESSAGES'))
      && args.toLowerCase() === 'force');
    if (force) {
      const result = await this.skip(msg.guild, queue);
